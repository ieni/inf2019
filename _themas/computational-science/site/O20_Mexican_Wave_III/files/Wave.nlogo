globals [sizes]
turtles-own [wave eigenAfstand]

to setup
  setup-circle
  ask patches [set pcolor white]
  ask turtles [
    set eigenAfstand 1 + random afstand
    set label-color black
    ifelse modelVariant = 3 [set label eigenAfstand] [set label afstand]
  ]
  reset-ticks
end
...
to setup-circle
  ...
  set sizes [2 2.5 3.5 5 7 5 3.5 2.5 2]

  create-ordered-turtles 40 [
    fd 20
  ]
  create-ordered-turtles 60 [
    fd 30
  ]
  create-ordered-turtles 80 [
    fd 40
  ]
  ...
  ask turtles [
    ...
    set wave 0
    rt 90]
  ask turtles with [xcor > 0 and ycor = 0] [set wave 1 set size item wave sizes ]
end

to go
  ask turtles [
    if modelVariant = 1 [variant1]
    if modelVariant = 2 [variant2]
    if modelVariant = 3 [variant3]
  ]
  ask turtles with [wave > 0] [
    set color 100 + wave
    set size item wave sizes
    if size = 7 [
      set color red
    ]
    set wave wave + 1
    if wave = 9 [
      set wave 0 set color orange
    ]
  ]
  tick
end
...
to variant1
   if size = 2 and max [size] of turtles in-cone (4 * afstand) (10 * afstand) > 2 [
      set wave 1
  ]
end

to variant2
  if size = 2 and max [size] of turtles with [subtract-headings heading [heading] of myself  < (8 * afstand) and subtract-headings heading [heading] of myself >= 0] > 2 [
      set wave 1
  ]
end

to variant3
  if size = 2 and max [size] of turtles with [subtract-headings heading [heading] of myself  < (8 * eigenAfstand) and subtract-headings heading [heading] of myself >= 0] > 2 [
      set wave 1
  ]
end
